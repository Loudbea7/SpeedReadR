...
from database.utils import create_db_and_tables, clear_database
...
class Speed_Read_R(MDApp):    
    def build(self):
        ...
        print("setting slot")
        # Clock.schedule_once(self._set_slot, 2) #### <<<< FIX THIS
        print("slot setted")
    ...
    def create_settings(self):
        ...
        print("After settings DAO")
        print("curr slot: ", self.active_dao.get_active().setting_active)
        print("sett: ", self.settings_dao.read_settings())
        print("curr sett: ", self.settings_dao.get_setting(self.active_dao.get_active().setting_active))
        print("theme dark: ", self.theme_dao.read_theme().dark)
        print("bookshelf: ", self.bookshelf_dao.read_bookshelf())
        print("curr slot: ", self.bookshelf_dao.get_book_title(self.active_dao.get_active().book))
        print("P TYPE: ", type(self.settings_dao.get_setting(self.active_dao.get_active().setting_active).pointer))
        print("P TYPE: ", self.settings_dao.get_setting(self.active_dao.get_active().setting_active).pointer)
    ...
    def update_WPM(self, *kwargs):
        ...
        if self.root_screen.ids.screen_manager.current == "sett":
            ...
            self.settings_screen.ids.wpm.text = str(self.settings_dao.get_setting(self.active_dao.get_active().setting_active).wpm)
        # self.reader_screen.ids.display_wpm.text = str(global_settings[current_slot]["wpm"])
        self.reader_screen.ids.display_wpm.text = str(self.settings_dao.get_setting(self.active_dao.get_active().setting_active).wpm)
    ...
    def post_settings(self, *kwargs):
        ...
        for key in set(self.settings_dao.get_setting(self.active_dao.get_active().setting_active)):
        # for key in settings if key != "active" else ...:
            ...
            try:
                if key not in switches:
                    ...
                    exec(f"self.settings_screen.{key}.text = str(int(self.settings_dao.get_setting(self.active_dao.get_active().setting_active).{key}))")
                else:
                    ...
                    exec(f"self.settings_screen.{key}.active = (True if self.settings_dao.get_setting(self.active_dao.get_active().setting_active).{key} == True else False)")
            except Exception as e:
                print("Error in post_settings: ", e)
        
        self.settings_screen.blink_color.md_bg_color = ast.literal_eval(self.settings_dao.get_setting(self.active_dao.get_active().setting_active).blink_color)
        self.posting = False
    ...
    def reader_sett_set_size(self, *kwargs):
        print("starting 1")
        self.settings_screen.ids.pager_sett_l.font_size = self.settings_dao.get_setting(self.active_dao.get_active().setting_active).reader_text_size
        print("starting 2")
        self.settings_screen.ids.pager_sett.font_size = self.settings_dao.get_setting(self.active_dao.get_active().setting_active).reader_text_size
        print("starting 3")
        self.settings_screen.ids.pager_sett_r.font_size = self.settings_dao.get_setting(self.active_dao.get_active().setting_active).reader_text_size
        print("starting 4")
        self.reader_screen.ids.pager_l.font_size = self.settings_dao.get_setting(self.active_dao.get_active().setting_active).reader_text_size
        print("starting 5")
        self.reader_screen.ids.pager.font_size = self.settings_dao.get_setting(self.active_dao.get_active().setting_active).reader_text_size
        print("starting 6")
        self.reader_screen.ids.pager_r.font_size = self.settings_dao.get_setting(self.active_dao.get_active().setting_active).reader_text_size
        print("starting 7")
    ...
    def show_pointer(self, *kwargs):
        ...
        if self.settings_dao.get_setting(self.active_dao.get_active().setting_active).pointer == True:
    ...
    def copy_clipboard(self, obj, site, *kwargs):
        erc20 = "0x177d49b81dED8Ff0CC59e8c1cc4F297e273f53D7"
    ...
    def speed_up(self):
        ...
        self.settings_dao.update_settings(slot=self.active_dao.get_active().setting_active, wpm=self.settings_dao.get_setting(self.active_dao.get_active().setting_active).wpm + 10)
    ...
    def set_timed(self):
        ...
        for s in t:
            self.timed[s] = int(global_settings[current_slot][s]) / 1000
        
        if self.settings_dao.get_setting(self.active_dao.get_active().setting_active).l_w_delay > 0:
            self.timed["step"] = 60 / self.settings_dao.get_setting(self.active_dao.get_active().setting_active).wpm / self.settings_dao.get_setting(self.active_dao.get_active().setting_active).l_w_delay / 10
            # timed["step"] = 60 / int(global_settings[current_slot]["wpm"]) / (int(global_settings[current_slot]["l_w_delay"]) / 10)
        
        if self.settings_dao.get_setting(self.active_dao.get_active().setting_active).l_w_delay <= 0:
            self.timed["step"] = 60 / self.settings_dao.get_setting(self.active_dao.get_active().setting_active).wpm
    ...
    def blink(self, start_time, blink_color, blinker):
        ...
        if self.settings_dao.get_setting(self.active_dao.get_active().setting_active).blink_fade_toggle == True and blink_color[3] > 0:
            blink_color[3] = 1 - ((time.time() - start_time) / self.settings_dao.get_setting(self.active_dao.get_active().setting_active).blink_fade)

        elif self.settings_dao.get_setting(self.active_dao.get_active().setting_active).blink_fade_toggle == True and blink_color[3] < 0:
            blink_color[3] = 0
    ...
    def save_settings(self, key, value, *kwargs):
        ...
        if self.posting:
            ...
            # print("All Executed")
            # self.settings_dao.update_settings(slot=self.active_dao.get_active().setting_active, key=value)
    ...
    def reload_bookshelf(self, *kwargs):
        ...
        files = listdir(self.active_dao.get_active().path)
        ...
        for book in files:
            ...
            if book_type in suported:
                ...
                if not old:
                    self.bookshelf_dao.update_bookshelf(title=book, hash=book_hash, type=book_type, indx=0, progress=0)
    ...
    def open_book(self, *kwargs):
        ...
        if type(kwargs[0]) == str:
            ...
            book_name = self.active_dao.get_active().book
        else:
            book_name = kwargs[0].id
            self.active_dao.update_active(book=book_name)
        pages = ""

        # book = db.execute("SELECT * FROM bookshelf WHERE title = ?", (book_name,)).fetchall()
        book = self.bookshelf_dao.get_book_title(title=book_name)
        # db.execute("UPDATE active SET book = ?", [book_name])
        # con.commit()
        
        # bk = ["id", "title", "hash", "type", "indx", "progress"]
        # open_book = {}
        # try:
        #     for b in range(len(bk)-1):
        #         # open_book[bk[b+1]] = book[0][b+1]
        #         exec(f"open_book[{bk[b+1]}] = book[{bk[b+1]}]")
        # except:
        #     return
        ...
        match book.type:
            case "text/plain":
                texted_book = Parsex.text(self.active_dao.get_active().path+book.title) # Read text
            case "application/epub+zip":
                texted_book = Parsex.ebook(self.active_dao.get_active().path+book.title) # Read text
            case "application/pdf":
                texted_book = Parsex.pdf(self.active_dao.get_active().path+book.title) # Read text
            case "text/markdown":
                texted_book = Parsex.md(self.active_dao.get_active().path+book.title) # Read text
            case "application/rtf":
                texted_book = Parsex.rtf(self.active_dao.get_active().path+book.title) # Read text
            case "application/x-tex":
                texted_book = Parsex.text(self.active_dao.get_active().path+book.title) # Read text
            case "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
                texted_book = Parsex.docx(self.active_dao.get_active().path+book.title) # Read text
        ...
        try:
            self.open_book_length = len(texted_book.split())
        except:
        ...
        self.word_index = texted_book.split()
        
        indx = int(open_book["indx"])

        # Text readed
        # cut = texted_book[:open_book["indx"]]
        cut = texted_book[:self.bookshelf_dao.get_book_title(title=book_name).indx]
        ...
        if self.settings_dao.get_setting(self.active_dao.get_active().setting_active).progress_bar:
            if self.index == 0:
                # open_book["progress"] = 0
                self.bookshelf_dao.update_bookshelf(title=book_name, progress=0)
                self.reader_screen.p_bar.value = 0
                self.reader_screen.index.text = str(self.index)
            else:
                # open_book["progress"] = self.index / self.open_book_length * 100
                self.bookshelf_dao.update_bookshelf(title=book_name, progress=self.index / self.open_book_length * 100)
                self.reader_screen.p_bar.value = self.bookshelf_dao.get_book_title(title=book_name).progress
                self.reader_screen.index.text = str(self.index)
        else:
            self.reader_screen.p_bar.value = 0
        ...
        tf = TextInput(
            ...
            font_size = self.settings_dao.get_setting(self.active_dao.get_active().setting_active).bg_text_size,
            multiline = True,
            readonly = True,
        ...
        Clock.schedule_once(
            lambda x: self.reader_screen.grid_pager.children[0].select_text(self.bookshelf_dao.get_book_title(title=book_name).indx, self.bookshelf_dao.get_book_title(title=book_name).indx+len(self.word_index[self.index])
            ))
    ...
    def click_text(self, *kwargs):
        ...
        if new_index != old_index:
            ...
            self.bookshelf_dao.update_bookshelf(title=self.active_dao.get_active().book, indx=self.indx)
            # db.execute("UPDATE bookshelf SET indx = ? WHERE title = ?", [indx, current_book])
            # con.commit()
            
            cut = texted_book[:self.indx]
            self.index = len(cut.split())
            self.bookshelf_dao.update_bookshelf(title=self.active_dao.get_active().book, progress= self.index / self.open_book_length * 100)
            # open_book["progress"] = self.index / self.open_book_length * 100
    ...
    def play_reader(self, *args):
        ...
        try:
            while self.play and self.index < word_count:
                ...
                delay(self.word_index[self.index], length)
